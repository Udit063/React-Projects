...
export const FormProvider = ({ children }) => {
  ...
  useEffect(() => {
    const autoSave = async () => {
      if (
        state.formId &&
        (Object.keys(state.formData).length > 0 || state.currentStep > 1)
      ) {
  ...
  const submitForm = async () => {
    ...
    if (Object.keys(allErrors).length > 0) {
      ...
      const firstErrorStep =
        allErrors.firstName || allErrors.lastName || allErrors.email ? 1 : 2;
  ...
  const resetForm = async () => {
    if (
      window.confirm(
        "Are you sure you want to reset this form? All data will be permanently deleted."
      )
    ) {
      try {
        if (state.formId) {
