...
class DynamoDBService {
  constructor() {
    ...
    this.checkAttempted = false; // avoid repeating failed checks continuously
    this.init();
  }
  async init() {
  ...
  async checkAndCreateTable() {
    // mark that we've attempted a table check so we don't loop endlessly
  ...
  async waitForTableActive() {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        console.log(
          `⏳ Waiting for table to be active... (${i + 1}/${maxAttempts})`
        );
  ...
  async ensureTableExists() {
    // Only attempt to check/create table once per service lifetime. If
    // the first attempt failed (for example due to missing/invalid
    // credentials), repeated attempts caused an error flood. By tracking
    // `checkAttempted` we avoid doing repeated immediate retries.
    if (!this.tableExists && !this.checkAttempted) {
      await this.checkAndCreateTable();
    }
